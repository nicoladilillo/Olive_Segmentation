...
def run_pipeline(input_path: str, output_dir: str):
    ...
    # 2) Telea inpainting
    clean_bgr = cv2.inpaint(bgr, specular, 5, cv2.INPAINT_TELEA)
    clean_rgb = cv2.cvtColor(clean_bgr, cv2.COLOR_BGR2RGB)

    # 3) Blur + Mean Shift
    blur = cv2.GaussianBlur(clean_bgr, (9, 9), 0)
    ms = cv2.pyrMeanShiftFiltering(blur, sp=12, sr=30, maxLevel=1)
    ms_rgb = cv2.cvtColor(ms, cv2.COLOR_BGR2RGB)

    # 4) Otsu on Lab b* channel
    lab = cv2.cvtColor(ms, cv2.COLOR_BGR2LAB)
    L, a, b = cv2.split(lab)

    otsu_threshold, base = cv2.threshold(
        b,
        0,
        255,
    ...
    # 5) Black-hat for dark gaps
    bh_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (13, 13))
    blackhat = cv2.morphologyEx(L, cv2.MORPH_BLACKHAT, bh_kernel)

    _, blackhat_mask = cv2.threshold(
        blackhat, 18, 255, cv2.THRESH_BINARY
    )

    blackhat_mask = cv2.morphologyEx(
        blackhat_mask, cv2.MORPH_OPEN, k3, iterations=1
    )
    ...
    edge_support = cv2.bitwise_or(strong_gradient, canny)

    # 7) Black-hat + derivative separators
    edge_near = cv2.dilate(edge_support, k5, iterations=1)
    separator = cv2.bitwise_and(blackhat_mask, edge_near)
    separator = cv2.dilate(separator, k3, iterations=1)

    # 8) Apply separators to Otsu mask
    separated = base.copy()
    separated[separator > 0] = 0
    separated = cv2.morphologyEx(
        separated, cv2.MORPH_OPEN, k3, iterations=1
    )
    ...
    distance_smooth = ndi.gaussian_filter(distance, sigma=2.2)
    ...
    # 10) Filter plausible olive regions
    ...
    # 11) Per-olive morphology + hole filling
    ...
    for obj_id in range(1, n + 1):
        ...
        allowable = cv2.dilate(crop, k3, iterations=2)
        filled = cv2.bitwise_and(filled, allowable)
    ...
    cv2.imwrite(str(output_dir / "03_mean_shift.png"), ms)
    cv2.imwrite(str(output_dir / "04_otsu_mask.png"), base)
    cv2.imwrite(str(output_dir / "05_blackhat.png"), blackhat)
    cv2.imwrite(str(output_dir / "06_separator.png"), separator)
    cv2.imwrite(
        str(output_dir / "07_final_mask.png"),
        (final > 0).astype(np.uint8) * 255
    )
    cv2.imwrite(
        str(output_dir / "08_final_overlay.png"),
    ...
    ax[0, 1].imshow(specular, cmap="gray")
    ...
    ax[1, 0].set_title(
        f"Otsu mask on Lab b* (T={otsu_threshold:.0f})"
    )
    ...
    comparison_path = output_dir / "09_pipeline_comparison.png"
    plt.savefig(comparison_path, dpi=180, bbox_inches="tight")
    plt.close(fig)

    print(f"Otsu threshold on Lab b*: {otsu_threshold:.0f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Olive segmentation with specular reflection removal, "
            "Mean Shift, Otsu, Black-hat, derivatives and Watershed."
        )
    )
    ...
    parser.add_argument(
        "--output",
        default=Path(__file__).resolve().parent / "results" / "otsu",
        help="Output directory. Default: computer_vision_pipeline/results/otsu"
    )
